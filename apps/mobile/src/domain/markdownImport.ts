...
import type { Goal, GoalStatus, Quest, QuestStatus, Skill, SkillStatus, SupportLevel, WeeklyReview } from "./types";
...
export function parseMarkdownBundle(markdown: string): ParsedDocument[] {
  ...
  for (const document of documents) {
    ...
    if (seen.has(key)) throw new Error(`Duplicate Markdown identity in import: ${key}`);
    seen.add(key);
  }
  return documents;
}
...
export async function importMarkdownBundle(repository: PersonalRepository, markdown: string, options: { defaultProfileId: string; displayName: string; importedAt: string }): Promise<MarkdownImportResult> {
  const documents = parseMarkdownBundle(markdown);
  if (documents.length === 0) throw new Error("No Levera Markdown documents found");
  ...
  const profileIds = new Set([options.defaultProfileId]);
  for (const document of documents) if (document.type === "goal" || document.type === "weekly_review") profileIds.add(stringField(document, "profile_id", options.defaultProfileId));
  ...
  for (const document of documents.filter((item) => item.type === "goal")) {
    const existing = await repository.getGoal(document.id); count(result, document.type, Boolean(existing));
    const updatedAt = stringField(document, "updated", options.importedAt);
    const goal: Goal = { id: document.id, profileId: stringField(document, "profile_id", options.defaultProfileId), title: document.title, description: descriptionBody(document.body), status: enumField(document, "status", ["active", "completed", "archived"] as GoalStatus[], "active"), createdAt: existing?.createdAt ?? updatedAt, updatedAt };
  ...
  for (const document of documents.filter((item) => item.type === "skill")) {
    ...
    const skill: Skill = { id: document.id, goalId, title: document.title, description: descriptionBody(document.body), supportLevel: numberField(document, "support_level", 0, 3, 2) as SupportLevel, status: enumField(document, "status", ["growing", "stable", "due", "fading", "paused"] as SkillStatus[], "growing"), masteryScore: numberField(document, "mastery_score", 0, 100, 0), nextReviewAt: optionalString(document, "next_review_at"), createdAt: existing?.createdAt ?? updatedAt, updatedAt };
    await repository.saveSkill(skill);
  }
  for (const document of documents.filter((item) => item.type === "quest")) {
    const skillIds = stringArrayField(document, "skill_ids"); if (skillIds.length === 0) throw new Error(`Imported quest ${document.id} has no skills`);
    for (const skillId of skillIds) if (!(await repository.getSkill(skillId))) throw new Error(`Imported quest ${document.id} references missing skill ${skillId}`);
...
function parseDocument(markdown: string): ParsedDocument {
  const match = markdown.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/); if (!match) throw new Error("Markdown document must start with YAML frontmatter");
  const fields = parseFrontmatter(match[1] ?? ""); const body = (match[2] ?? "").trim(); const id = asString(fields.levera_id); const type = asString(fields.levera_type);
  if (!id) throw new Error("Markdown document is missing levera_id"); if (!isType(type)) throw new Error(`Unsupported levera_type: ${type || "missing"}`);
  const title = body.match(/^#\s+(.+)$/m)?.[1]?.trim() || id;
  return { id, type, title, body, fields };
}
