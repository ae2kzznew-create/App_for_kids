import type { ExternalNoteEntityType, ExternalNoteLink, Goal, Profile, ProgressEvent, Quest, QuestCompletion, QuestSkill, Skill, SkillEdge, WeeklyReview } from "./types";
...
export interface PersonalRepository {
  ...
  getWeeklyReview(id: string): Promise<WeeklyReview | null>; getWeeklyReviewByWeek(profileId: string, weekStart: string): Promise<WeeklyReview | null>; saveImportedWeeklyReview(review: WeeklyReview): Promise<void>; saveWeeklyReviewWithEvent(review: WeeklyReview, event: ProgressEvent): Promise<void>; listWeeklyReviews(profileId: string, limit?: number): Promise<WeeklyReview[]>; listProgressEvents(entityId: string): Promise<ProgressEvent[]>;
  getExternalNoteLink(entityType: ExternalNoteEntityType, entityId: string): Promise<ExternalNoteLink | null>; saveExternalNoteLink(link: ExternalNoteLink): Promise<void>; deleteExternalNoteLink(entityType: ExternalNoteEntityType, entityId: string): Promise<void>;
}
