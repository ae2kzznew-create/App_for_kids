...
export class SQLitePersonalRepository implements PersonalRepository {
  ...
  async ensureProfile(profile: Profile) { await this.db.runAsync("INSERT INTO profiles (id, display_name, created_at, updated_at) VALUES (?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET display_name = excluded.display_name, updated_at = excluded.updated_at", profile.id, profile.displayName, profile.createdAt, profile.updatedAt); }
  async getGoal(id: string) { const row = await this.db.getFirstAsync<GoalRow>("SELECT * FROM goals WHERE id = ?", id); return row ? mapGoal(row) : null; }
  async saveGoal(goal: Goal) { await this.db.runAsync("INSERT INTO goals (id, profile_id, title, description, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET title = excluded.title, description = excluded.description, status = excluded.status, updated_at = excluded.updated_at", goal.id, goal.profileId, goal.title, goal.description ?? null, goal.status, goal.createdAt, goal.updatedAt); }
  async listGoals(profileId: string) { const rows = await this.db.getAllAsync<GoalRow>("SELECT * FROM goals WHERE profile_id = ? ORDER BY updated_at DESC", profileId); return rows.map(mapGoal); }
  async getSkill(id: string) { const row = await this.db.getFirstAsync<SkillRow>("SELECT * FROM skills WHERE id = ?", id); return row ? mapSkill(row) : null; }
  async saveSkill(skill: Skill) { await this.db.runAsync("INSERT INTO skills (id, goal_id, title, description, support_level, status, mastery_score, next_review_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET title = excluded.title, description = excluded.description, support_level = excluded.support_level, status = excluded.status, mastery_score = excluded.mastery_score, next_review_at = excluded.next_review_at, updated_at = excluded.updated_at", skill.id, skill.goalId, skill.title, skill.description ?? null, skill.supportLevel, skill.status, skill.masteryScore, skill.nextReviewAt ?? null, skill.createdAt, skill.updatedAt); }
  async listSkills(goalId: string) { const rows = await this.db.getAllAsync<SkillRow>("SELECT * FROM skills WHERE goal_id = ? ORDER BY updated_at DESC", goalId); return rows.map(mapSkill); }
  ...
  async listQuestSkills(questId: string) { return this.db.getAllAsync<QuestSkill>("SELECT quest_id AS questId, skill_id AS skillId, contribution_weight AS contributionWeight FROM quest_skills WHERE quest_id = ?", questId); }
  async listActiveQuests() { const rows = await this.db.getAllAsync<QuestRow>("SELECT * FROM quests WHERE status = 'active' ORDER BY scheduled_for IS NULL, scheduled_for, updated_at DESC"); return rows.map(mapQuest); }
  ...
  async listCompletions(questId: string) { const rows = await this.db.getAllAsync<CompletionRow>("SELECT * FROM quest_completions WHERE quest_id = ? ORDER BY completed_at DESC", questId); return rows.map(mapCompletion); }
  async listCompletedQuests(limit = 50) { return this.queryHistory("", [], limit); }
  async listSkillHistory(skillId: string, limit = 50) { return this.queryHistory("WHERE EXISTS (SELECT 1 FROM quest_skills filter_qs WHERE filter_qs.quest_id = q.id AND filter_qs.skill_id = ?)", [skillId], limit); }
  async listProgressEvents(entityId: string) { const rows = await this.db.getAllAsync<EventRow>("SELECT * FROM progress_events WHERE entity_id = ? ORDER BY occurred_at DESC", entityId); return rows.map(mapEvent); }

  private async queryHistory(where: string, params: Array<string | number>, limit: number): Promise<CompletedQuestSummary[]> {
    const safeLimit = Math.max(1, Math.min(200, Math.floor(limit)));
    const rows = await this.db.getAllAsync<HistoryRow>(`SELECT q.id AS quest_id, q.title, c.completed_at, c.xp_granted, c.evidence_note, c.reflection, GROUP_CONCAT(qs.skill_id) AS skill_ids FROM quest_completions c JOIN quests q ON q.id = c.quest_id LEFT JOIN quest_skills qs ON qs.quest_id = q.id ${where} GROUP BY c.id, q.id ORDER BY c.completed_at DESC LIMIT ?`, ...params, safeLimit);
    return rows.map((row) => ({ questId: row.quest_id, title: row.title, completedAt: row.completed_at, xpGranted: row.xp_granted, evidenceNote: row.evidence_note ?? undefined, reflection: row.reflection ?? undefined, skillIds: row.skill_ids ? row.skill_ids.split(",") : [] }));
  }
}
...
type HistoryRow = { quest_id: string; title: string; completed_at: string; xp_granted: number; evidence_note: string | null; reflection: string | null; skill_ids: string | null };
